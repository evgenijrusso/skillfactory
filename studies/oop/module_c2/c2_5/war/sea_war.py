...
class Ship:
    def __init__(self, dota: Dot, length: int, vertical: bool):
        self.dota = dota
        self.length = length
...
class Player:
    ...
    def move(self) -> bool:    # возвращает True, если нужно ход повторить
        while True:
            try:
                target = self.ask() #  нет запроса, будет ошибка
...
class Game:
    ...
    def print_boards(self):  # вывод двух досок рядом по горизонтали
        print('-' * self.size * 10)
        print('Моя доска:'.ljust((self.size + 1) * 4 - 1) + ' ' * self.size + 'Доска компьютера:')
